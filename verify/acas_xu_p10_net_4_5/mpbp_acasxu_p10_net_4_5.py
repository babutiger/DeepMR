...
class network(object):
    ...
    def find_robustness_number_mpbp(self, PROPERTY, t, TRIM=False):
        ...
        for neuron_i in self.layers[-1].neurons:  # 这么写表示和其他数字有交集？判断网络f可达集与不安全区域S- 是否有交集？
            ...
            if neuron_i.concrete_lowest_upper_global > 0:  # 这是unkown鲁棒不鲁棒!  表示 网络f可达集与不安全区域S- 有 交集！
...
def mnist_robustness_radius_mpbp():
    ...
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_20x50_mpbp_radius_result_" + str(style_time) + ".txt", mode="w+",
                encoding="utf-8")
...
def mnist_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_20x50_deepmr_radius_result_" + str(style_time) + ".txt", mode="w+",
                encoding="utf-8")
...
def test_robustness_number_mpbp(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open(f"../../result/original_result/acas_xu_p10_net_4_5_mpbp_number_result_delta_{d}_{style_time}.txt",
...
if __name__ == "__main__":
    ...
    # test_robustness_number_mpbp(0)
    test_robustness_number_mpbp(12)

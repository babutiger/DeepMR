...
class network(object):
    ...
    def verify_lp_split(self, PROPERTY, DELTA, MAX_ITER=5, SPLIT_NUM=0, WORKERS=12, TRIM=False, SOLVER=cp.GUROBI,
        ...
        delta_list = [
            self.layers[0].neurons[i].concrete_lowest_upper_polylocal - self.layers[0].neurons[
                i].concrete_highest_lower_polylocal for
        ...
        print('Overall Unsafe Overapproximate(MPBP_algorithm)%:',
              len(unsafe_set_mpbp_algorithm) / len(split_list) * 100)
        if MODE == self.MODE_ROBUSTNESS:
            return True
    ...
    def find_robustness_number_mrlp(self, PROPERTY, t, TRIM, WORKERS=12, SOLVER=cp.GUROBI):
...
def mnist_robustness_radius_mpbp():
    ...
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_20x50_mpbp_radius_result_" + str(style_time) + ".txt",
                mode="w+",
                encoding="utf-8")
...
def mnist_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_20x50_deepmr_radius_result_" + str(style_time) + ".txt",
                mode="w+",
                encoding="utf-8")
...
def test_robustness_number_mrlp(d):
    ...
    net = network()
    net.load_nnet("../../models/nnet/ACASXU_run2a_4_5_batch_2000.nnet")
    property_list = [f"../../acas_properties/acas_xu_p10_net_4_5_new/local_robustness_{i}.txt" for i in range(1, amount+1)]

    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open(f"../../result/original_result/acas_xu_p10_net_4_5_deepmr_number_result_delta_{d}_{style_time}.txt", mode="w+", encoding="utf-8")
...
if __name__ == "__main__":
    ...
    # test_robustness_number_mrlp(0.005)
    # test_robustness_number_mrlp(0.8)
    test_robustness_number_mrlp(8)

...
class network(object):
    ...
    def verify_lp_split(self, PROPERTY, DELTA, MAX_ITER=5, SPLIT_NUM=0, WORKERS=28, TRIM=False, SOLVER=cp.GUROBI,
        ...
        for j in range(MAX_ITER):
            ...
            for neuron_i in self.layers[-1].neurons:  # 这么写表示和其他数字有交集？判断网络f可达集与不安全区域S- 是否有交集？
                ...
                if neuron_i.concrete_lowest_upper > 0:  # 这是unkown鲁棒不鲁棒!  表示 网络f可达集与不安全区域S- 有 交集！
            ...
            if flag == True:  # 这是鲁棒safe! 网络f可达集与不安全区域S- 没有 交集！
                ...
                # 大M法
                # 自适应 Big-M: M 必须 >= 约束表达式在可行域上的上界(Big-M有效性标准).
                # 约束 variables[-1][i] >= -M(1-z_i); z_i=0 需非绑定 => M >= max_i(-下界(margin_i)).
                # margin 下界由 DeepPoly 当前抽象给出(每轮自适应收紧); 2x 安全系数; BIGM 环境变量可覆盖(敏感性分析用).
    ...
    def deepzono(self):
        """Zonotope (DeepZono) forward propagation.
        Stores an affine form (zc = center, zg = generator coefficients over a
        global noise-symbol vector) plus concretized bounds (zlo, zhi) and a
        crossing flag (cf) on every neuron. Separate attributes from the DeepPoly
        ones so both abstractions can run on the same net object."""
        n_relu = sum(l.size for l in self.layers if l.layer_type == layer.RELU_LAYER)
        N = self.inputSize + n_relu
        ne = self.inputSize  # next free fresh-noise index (one per crossing ReLU)
        L0 = self.layers[0]
        for i in range(L0.size):
            neu = L0.neurons[i]
            lo = neu.concrete_lower; hi = neu.concrete_upper
            neu.zc = 0.5 * (lo + hi)
            g = np.zeros(N); g[i] = 0.5 * (hi - lo)
            neu.zg = g; neu.zlo = lo; neu.zhi = hi; neu.cf = 0
        for k in range(1, len(self.layers)):
            cur = self.layers[k]; pre = self.layers[k - 1]
            if cur.layer_type == layer.AFFINE_LAYER:
                PC = np.array([nu.zc for nu in pre.neurons])
                PG = np.stack([nu.zg for nu in pre.neurons])  # (pre.size, N)
                for neu in cur.neurons:
                    w = neu.weight
                    neu.zc = float(w @ PC + neu.bias)
                    neu.zg = w @ PG
                    rad = np.abs(neu.zg).sum()
                    neu.zlo = neu.zc - rad; neu.zhi = neu.zc + rad; neu.cf = 0
            elif cur.layer_type == layer.RELU_LAYER:
                for j in range(cur.size):
                    neu = cur.neurons[j]; p = pre.neurons[j]
                    l, u = p.zlo, p.zhi
                    # refinement feedback: use spurious-region-tightened pre-activation
                    # bounds (valid within the spurious region) to tighten the ReLU
                    # transformer, exactly as DeepMR feeds tightened bounds back to DeepPoly.
                    if getattr(p, 'tl', None) is not None: l = max(l, p.tl)
                    if getattr(p, 'tu', None) is not None: u = min(u, p.tu)
                    if l >= 0:
                        neu.zc = p.zc; neu.zg = p.zg.copy(); neu.zlo = l; neu.zhi = u; neu.cf = 1
                    elif u <= 0:
                        neu.zc = 0.0; neu.zg = np.zeros(N); neu.zlo = 0.0; neu.zhi = 0.0; neu.cf = 2
                    else:
                        lam = u / (u - l)
                        mu = -l * u / (2.0 * (u - l))   # >= 0 ; half the parallelogram height
                        neu.zc = lam * p.zc + mu
                        g = lam * p.zg.copy(); g[ne] = mu; ne += 1
                        neu.zg = g
                        rad = np.abs(g).sum(); neu.zlo = neu.zc - rad; neu.zhi = neu.zc + rad; neu.cf = 0
        self._N = N

    def zono_forward_concrete(self, x):
        """Exact network forward pass through the loaded layers (incl. the appended
        verify/unsafe-halfspace layer). Used only for the soundness sanity check."""
        vals = np.array(x, dtype=float)
        for k in range(1, len(self.layers)):
            cur = self.layers[k]
            if cur.layer_type == layer.AFFINE_LAYER:
                W = np.stack([nu.weight for nu in cur.neurons])
                b = np.array([nu.bias for nu in cur.neurons])
                vals = W @ vals + b
            elif cur.layer_type == layer.RELU_LAYER:
                vals = np.maximum(vals, 0.0)
        return vals

    def zono_soundness_check(self, n_samples=2000, seed_pts=None):
        """Sample random inputs in the box; verify every verify-layer halfspace's
        concrete value lies within [zlo, zhi] of its zonotope affine form."""
        L0 = self.layers[0]
        los = np.array([nu.concrete_lower for nu in L0.neurons])
        his = np.array([nu.concrete_upper for nu in L0.neurons])
        vlayer = self.layers[-1]
        vlo = np.array([nu.zlo for nu in vlayer.neurons])
        vhi = np.array([nu.zhi for nu in vlayer.neurons])
        rng = np.random.RandomState(12345)
        worst = 0.0
        pts = [los, his, 0.5 * (los + his)]
        for _ in range(n_samples):
            pts.append(los + (his - los) * rng.rand(len(los)))
        for x in pts:
            v = self.zono_forward_concrete(x)
            over = float(np.max(v - vhi)); under = float(np.max(vlo - v))
            worst = max(worst, over, under)
        return worst  # should be <= ~0 (tiny positive = float noise) if sound

    def verify_zono(self, PROPERTY, DELTA, MAX_ITER=5, SOLVER=cp.GUROBI, verbose=True, WORKERS=8):
        """DeepMR refinement on a DeepZono abstraction: Big-M simultaneous
        spurious-region elimination, spurious-region-guided bound tightening, and
        re-propagation of the tightened bounds (the same refinement loop DeepMR
        runs on DeepPoly). Returns True iff the robustness property is verified."""
        if self.property_flag == True:
            self.layers.pop(); self.property_flag = False
        self.load_robustness(PROPERTY, DELTA)
        for lyr in self.layers:
            for nu in lyr.neurons:
                nu.tl = None; nu.tu = None; nu.cf = 0
        base_reported = False
        for it in range(MAX_ITER + 1):
            self.deepzono()  # re-propagate with the current tightened bounds
            vlayer = self.layers[-1]
            vc = np.array([nu.zc for nu in vlayer.neurons])
            vg = np.stack([nu.zg for nu in vlayer.neurons])
            vlo = np.array([nu.zlo for nu in vlayer.neurons])
            vhi = np.array([nu.zhi for nu in vlayer.neurons])
            verify_list = [i for i in range(vlayer.size) if vhi[i] >= 0]
            if not base_reported:
                if verbose:
                    print("  [zono] base DeepZono: max halfspace upper = %.4f, #possibly-unsafe = %d"
                          % (vhi.max(), len(verify_list)))
                base_reported = True
            if len(verify_list) == 0:
                if verbose:
                    print("  [zono] Verified after %d refinement iteration(s)" % it if it > 0
                          else "  [zono] Verified by base DeepZono abstraction alone")
                return True
            uncertain = []
            for k in range(len(self.layers) - 1):
                cur = self.layers[k]; nxt = self.layers[k + 1]
                if cur.layer_type == layer.AFFINE_LAYER and nxt.layer_type == layer.RELU_LAYER:
                    for j in range(cur.size):
                        if nxt.neurons[j].cf == 0:
                            uncertain.append(cur.neurons[j])
            if it == 0 and verbose:
                print("  [zono] refinement: %d crossing ReLU neurons, %d noise symbols" % (len(uncertain), self._N))
            if it == MAX_ITER:
                if verbose: print("  [zono] still feasible after %d iterations -> Unverified" % MAX_ITER)
                return False
            N = self._N
            Mbig = 2.0 * max([-vlo[i] for i in verify_list] + [1.0])
            eps = cp.Variable(N)
            z = [cp.Variable(boolean=True) for _ in verify_list]
            cons = [eps >= -1, eps <= 1]
            for nn in uncertain:
                aff = nn.zc + nn.zg @ eps
                lo = nn.tl if nn.tl is not None else nn.zlo
                hi = nn.tu if nn.tu is not None else nn.zhi
                cons.append(aff >= lo); cons.append(aff <= hi)
            for t, i in enumerate(verify_list):
                cons.append(vc[i] + vg[i] @ eps >= -Mbig * (1 - z[t]))
            cons.append(cp.sum(z) >= 1)
            prob = cp.Problem(cp.Maximize(0), cons)
            prob.solve(solver=SOLVER)
            if prob.status not in (cp.OPTIMAL, cp.OPTIMAL_INACCURATE):
                if verbose: print("  [zono] iter %d: Big-M MILP infeasible -> Verified" % it)
                return True
            # tighten every uncertain pre-activation bound over the spurious region (parallel)
            tasks = []
            for nn in uncertain:
                aff = nn.zc + nn.zg @ eps
                tasks.append((None, cons, cp.Minimize(aff), SOLVER))
                tasks.append((None, cons, cp.Maximize(aff), SOLVER))
            pool = mp.Pool(WORKERS)
            res = pool.starmap(lpsolve, tasks)
            pool.terminate()
            improved = 0
            for idx, nn in enumerate(uncertain):
                lo = res[2 * idx]; hi = res[2 * idx + 1]
                cur_lo = nn.tl if nn.tl is not None else nn.zlo
                cur_hi = nn.tu if nn.tu is not None else nn.zhi
                if lo is not None and lo > cur_lo + 1e-7: nn.tl = lo; improved += 1
                if hi is not None and hi < cur_hi - 1e-7: nn.tu = hi; improved += 1
            if verbose: print("  [zono] iter %d: feasible; tightened %d pre-activation bounds" % (it, improved))
            if improved == 0:
                if verbose: print("  [zono] no further tightening -> Unverified")
                return False
        return False
...
def zono_demo(prop_idx=0, deltas=None, refine_delta=None, max_iter=3):
    NNET = "../../models/mnist_new_5x50/mnist_net_new_5x50.nnet"
    PROP = "../../mnist_properties/mnist_properties_5x50/mnist_property_%d.txt" % prop_idx
    if deltas is None:
        deltas = [0.004, 0.006, 0.008, 0.010, 0.012]

    # --- soundness sanity check ---
    net = network(); net.load_nnet(NNET)
    net.load_robustness(PROP, 0.02); net.deepzono()
    worst = net.zono_soundness_check(n_samples=3000)
    print("=== SOUNDNESS CHECK (delta=0.02, 3003 samples) ===")
    print("worst zonotope over/under-approximation violation: %.3e  (<=~0 means sound)" % worst)
    assert worst < 1e-6, "UNSOUND zonotope! violation=%.3e" % worst
    print("OK: zonotope soundly over-approximates the network on all samples.\n")

    # --- base DeepZono scan ---
    print("=== BASE DeepZono (abstraction only, no refinement) — property %d ===" % prop_idx)
    for d in deltas:
        net = network(); net.load_nnet(NNET)
        net.property_flag = False
        v = net.verify_zono(PROP, d, MAX_ITER=0)  # 0 iters -> base only
        print("  delta=%.4f -> %s\n" % (d, "VERIFIED" if v else "not (needs refinement)"))

    # --- DeepZono-DeepMR (refinement) vs DeepPoly-DeepMR at one delta ---
    if refine_delta is not None:
        print("=== DeepZono-DeepMR (Big-M refinement) vs DeepPoly-DeepMR @ delta=%.4f ===" % refine_delta)
        t0 = time.time()
        net = network(); net.load_nnet(NNET); net.property_flag = False
        vz = net.verify_zono(PROP, refine_delta, MAX_ITER=max_iter)
        tz = time.time() - t0
        print("  --> DeepZono-DeepMR: %s  (%.1fs)\n" % ("VERIFIED" if vz else "UNVERIFIED", tz))
        t0 = time.time()
        net2 = network(); net2.load_nnet(NNET); net2.property_flag = False
        vp = net2.verify_lp_split(PROPERTY=PROP, DELTA=refine_delta, MAX_ITER=5, SPLIT_NUM=0,
                                  WORKERS=8, TRIM=False, SOLVER=cp.GUROBI, MODE=1)
        tp = time.time() - t0
        print("  --> DeepPoly-DeepMR (paper): %s  (%.1fs)" % ("VERIFIED" if vp else "UNVERIFIED", tp))
...
if __name__ == "__main__":
    ...
    if len(args) >= 1 and args[0] == "scan":
        zono_demo(prop_idx=int(args[1]) if len(args) > 1 else 0)
    elif len(args) >= 2 and args[0] == "refine":
        zono_demo(prop_idx=int(args[1]), deltas=[float(args[2])] if len(args) > 2 else None,
                  refine_delta=float(args[2]) if len(args) > 2 else 0.01,
                  max_iter=int(args[3]) if len(args) > 3 else 3)
    else:
        zono_demo(prop_idx=0)

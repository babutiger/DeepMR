...
from models.mnist_new_9x200.mnist_net_9x200_model import *  # Import the created network model
...
def test_robustness_number_acrown(d):
    ...
    model = NeuralNetwork()
    checkpoint = torch.load("../../models/mnist_new_9x200/mnist_net_new_9x200.pth", map_location=torch.device('cpu'))
    model.load_state_dict(checkpoint)
    ...
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    # file = open("../../result/original_result/mnist_new_9x100_alpha_crown_radius_result_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")
    file = open("../../result/original_result/mnist_new_9x200_alpha_crown_number_result_delta_"+ str(d) +"_"+ str(style_time) + ".txt", mode="w+", encoding="utf-8")

import sys as _sys
MAXIT = int(_sys.argv[1]) if len(_sys.argv) > 1 else 50
import os
...
class network(object):
    ...
    def find_robustness_number_mrlp(self, PROPERTY, t, TRIM, WORKERS=28, SOLVER=cp.GUROBI):
        if self.verify_lp_split(PROPERTY=PROPERTY, DELTA=t, MAX_ITER=MAXIT, SPLIT_NUM=0, WORKERS=WORKERS,
                                TRIM=TRIM, SOLVER=SOLVER, MODE=1):
...
def test_robustness_number_mrlp(d):
    ...
    property_list = [f"../../acas_properties/acas_xu_p6_net_1_1/local_robustness_{i}.txt" for i in [28, 29]]

    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open(f"../../result/original_result/acas_xu_p6_net_1_1_deepmr_3_maxiter50_gap_number_result_delta_{d}_{style_time}.txt", mode="w+", encoding="utf-8")
...
if __name__ == "__main__":
    ...
    # test_robustness_number_srgrlp(0.005)

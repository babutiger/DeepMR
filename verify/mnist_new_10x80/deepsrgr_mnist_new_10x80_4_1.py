...
def lpsolve(vars,cons,obj,solver=cp.CBC):
...
class network(object):
    ...
    def verify_lp_split(self,PROPERTY,DELTA,MAX_ITER=5,SPLIT_NUM=0,WORKERS=12,TRIM=False,SOLVER=cp.CBC,MODE=0,USE_OPT_2=False):
    ...
    def find_max_disturbance_lp(self,PROPERTY,L,R,TRIM,WORKERS=12,SOLVER=cp.CBC):
        ans=L
        while L<=R:
            mid=int((L+R)/2)
    ...
    def find_robustness_number_srgrlp(self, PROPERTY, t, TRIM, WORKERS=12, SOLVER=cp.CBC):

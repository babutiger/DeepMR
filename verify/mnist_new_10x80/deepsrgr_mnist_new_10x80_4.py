# 先求第二层，再求输入层
# 把final输出层析取关系，精化输入层，用大M法处理逻辑关系的线性化
# 成功了！，这是改进后的版本，deepmr就是再加上多路径算法就好了。
import psutil
import os
...
# a = 0
# n = 16
# number_list = list(range(a*n, (a+1)*n))
# p.cpu_affinity(number_list)
...
class network(object):
    ...
    def verify_lp_split(self,PROPERTY,DELTA,MAX_ITER=5,SPLIT_NUM=0,WORKERS=30,TRIM=False,SOLVER=cp.GUROBI,MODE=0,USE_OPT_2=False):
        ...
        for j in range(MAX_ITER):
            ...
            if flag == True:  # 这是鲁棒safe! 网络f可达集与不安全区域S- 没有 交集！
                ...
                for k in range(2):
                    ...
                    if cur_layer.layer_type==layer.INPUT_LAYER:
                        for p in range(cur_layer.size):
                            constraints_inputlayer.append(variables[k][p]>=cur_neuron_list[p].concrete_lower)
                            constraints_inputlayer.append(variables[k][p]<=cur_neuron_list[p].concrete_upper)
                    elif cur_layer.layer_type==layer.AFFINE_LAYER:
                        assert(k>0)
                        for p in range(cur_layer.size):
                            constraints_inputlayer.append(variables[k][p]==cur_neuron_list[p].weight@variables[k-1]+cur_neuron_list[p].bias)
                            constraints_inputlayer.append(variables[k][p]>=cur_neuron_list[p].concrete_lower)
                            constraints_inputlayer.append(variables[k][p]<=cur_neuron_list[p].concrete_upper)
        ...
        # else:
        #     if MODE == self.MODE_ROBUSTNESS:
        #         # print("MODE==self.MODE_ROBUSTNESS")
        #         # print("MODE:", MODE)
        #         return True
    ...
    def find_max_disturbance_lp(self,PROPERTY,L,R,TRIM,WORKERS=30,SOLVER=cp.GUROBI):
        ans=L
        while L<=R:
            mid=int((L+R)/2)
    ...
    def find_robustness_number_srgrlp(self, PROPERTY, t, TRIM, WORKERS=30, SOLVER=cp.GUROBI):
...
def mnist_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_10x80_deepsrgr_radius_result_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")
    ...
    for property_i in property_list:
        ...
        delta_base = net.find_max_disturbance_lp(PROPERTY=property_i,L=int(delta_base*1000),R=int(delta_base*1000+63),TRIM=True,WORKERS=30,SOLVER=cp.CBC)
        end_time = time.time()
        property_i = property_i[46:]
        property_i = property_i[:-4]
        print(f"{property_i} -- delta_base : {delta_base}")
...
def test_robustness_number_srgrlp(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/mnist_new_10x80_deepsrgr_number_result_delta_" + str(d) + "_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")

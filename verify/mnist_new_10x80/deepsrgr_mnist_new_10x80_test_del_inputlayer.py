...
class network(object):
    ...
    def verify_lp_split(self,PROPERTY,DELTA,MAX_ITER=5,SPLIT_NUM=0,WORKERS=28,TRIM=False,SOLVER=cp.GUROBI,MODE=0,USE_OPT_2=False):
...
def test_robustness_number_srgrlp(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/mnist_new_10x80_deepsrgr_del_inputlayer_number_result_delta_" + str(d) + "_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")

...
from models.mnist_new_10x80.mnist_net_10x80_model import *  # Import the created network model
...
def test_robustness_number_acrown(d):
    ...
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    # file = open("../../result/original_result/mnist_new_10x80_alpha_crown_radius_result_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")
    file = open("../../result/original_result/mnist_new_10x80_alpha_crown_number_result_delta_"+ str(d) +"_"+ str(style_time) + ".txt", mode="w+", encoding="utf-8")
...
if __name__ == "__main__":
    test_robustness_number_acrown(0.015)

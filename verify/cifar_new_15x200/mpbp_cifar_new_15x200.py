...
def cifar_robustness_radius_mpbp():
    ...
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_15x200_mpbp_radius_result_" + str(style_time) + ".txt", mode="w+",
                encoding="utf-8")
...
def cifar_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_15x200_deepmr_radius_result_" + str(style_time) + ".txt", mode="w+",
                encoding="utf-8")

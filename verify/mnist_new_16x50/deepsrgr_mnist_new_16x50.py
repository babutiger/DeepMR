...
def mnist_robustness_radius_lp():
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/mnist_new_16x50_deepsrgr_radius_result_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")
...
def test_robustness_number_srgrlp(d):
    ...
    if not os.path.isdir('../../result/original_result'):
        os.makedirs('../../result/original_result')
    file = open("../../result/original_result/mnist_new_16x50_deepsrgr_number_result_delta_" + str(d) + "_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")

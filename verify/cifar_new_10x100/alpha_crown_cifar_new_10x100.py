...
from models.cifar_new_10x100.cifar_net_10x100_model import *  # Import the created network model
...
def cifar_robustness_radius():
    ...
    style_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
    file = open("../../result/original_result/cifar_new_10x100_alpha_crown_radius_result_" + str(style_time) + ".txt", mode="w+", encoding="utf-8")
...
def test_robustness_number_acrown(d):
    ...
    file = open("../../result/original_result/cifar_new_10x100_alpha_crown_number_result_delta_"+ str(d) +"_"+ str(style_time) + ".txt", mode="w+", encoding="utf-8")
...
if __name__ == "__main__":
    test_robustness_number_acrown(0.0027)

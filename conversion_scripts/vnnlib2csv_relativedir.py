...
def save_vnnlib_paths_to_csv(input_dir, output_csv_path):
    ...
    vnnlib_files = sorted(
        [os.path.join(input_dir, filename) for filename in os.listdir(input_dir) if filename.endswith('.vnnlib')],
...
# Example usage
save_vnnlib_paths_to_csv("../mnist_vnnlib/mnist_10x80_vnnlib", "../vnnlib_file_paths_relativedir.csv")
